...
public:
	...
	void set(int i, int j, const float v);
...
void CreateRectangularPrism(Scene& scene, float l, float w, float h);
void CreateCone(Scene& scene, float r, float l, float count);
void CreateCylinder(Scene& scene, float r, float l, float count);
...
void DrawMessage(const char* msg, int line=1);
...
#ifdef VIEWCELLS
void Interact(Scene& scene, int mx, int my, bool down);
#endif

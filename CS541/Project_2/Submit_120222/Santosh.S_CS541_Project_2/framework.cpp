...
void KeyboardInput(unsigned char key, int, int)
{
	...
	case 'm':					// Toggle between two models, 0 and 1
		scene.model = 1-scene.model;
		glutIdleFunc(&ReDraw);
		break;

	case 's':					// Toggle shaders on/off
		scene.UseShader = !scene.UseShader;
		glutIdleFunc(&ReDraw);
		break;
    ...
    case 27:                    // Exit program, Q key or Escape key
    case 'q':
        exit(0);
    }
}
...
int main(int argc, char** argv)
{
    ...
    glutInit(&argc, argv);
    glutCreateWindow("CS541 Framework");

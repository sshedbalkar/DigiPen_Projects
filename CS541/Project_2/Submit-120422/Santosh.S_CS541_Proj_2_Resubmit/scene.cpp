...
void InitializeScene(Scene &scene)
{
    ...
    FILE *f;
	unsigned char* tMap = new unsigned char[256*256*3];
	unsigned char* hMap = new unsigned char[256*256*1];

	if(f=fopen("tile_color.raw","rb"))
	{
		fread(tMap,256*256,3,f);
		fclose(f);
	}

	if(f=fopen("tile_height.raw","rb"))
	{
		fread(hMap,256*256,1,f);
		fclose(f);
	}
    ...
    gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGB, 256,256,GL_RGB, GL_UNSIGNED_BYTE, tMap);
    ...
    glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    gluBuild2DMipmaps(GL_TEXTURE_2D, GL_LUMINANCE, 256,256,GL_LUMINANCE, GL_UNSIGNED_BYTE, hMap);
    glBindTexture(GL_TEXTURE_2D,0);
	//
	delete[] tMap;
	delete[] hMap;

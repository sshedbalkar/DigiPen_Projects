 ...
 */
/*
	\class EventJobFinished
	\brief This is a class when is an event object created by PrintQueue. It holds information about the finished job and printer
*/
class EventJobFinished : public TimedCommand {
	PrintJob job;		///< The job just finished
	int printer_id;		///< ID of the Priniter which finished the job
	PrintQueue* p_print_queue;		///< The PrintQueue which completed the job
	void(PrintQueue::*p_method)(double, int, PrintJob);		///< Pointer to the PrintQueue method which is to be called by the simulator when the job is finished
public:
	EventJobFinished(double when, const PrintJob& _job, int _printer_id, PrintQueue* _p_print_queue, void(PrintQueue::*_p_method)(double, int, PrintJob));

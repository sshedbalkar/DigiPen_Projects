...
// State machines
//#include "example.h"
#include "SM_Runner_1.h"
#include "SM_Runner_2.h"
...
void World::Initialize( CMultiAnim *pMA, std::vector< CTiny* > *pv_pChars, CSoundManager *pSM, double dTimeCurrent )
{
	...
	for( int i=0; i<13; i++ )
	{
		...
		if(i < 4)
			npc->GetStateMachineManager()->PushStateMachine( *new SM_Runner_1( *npc, i/4, i%4 ), STATE_MACHINE_QUEUE_0, TRUE );
		else if(i < 8)
			npc->GetStateMachineManager()->PushStateMachine( *new SM_Runner_2( *npc, i/4, i%4 ), STATE_MACHINE_QUEUE_0, TRUE );
		else if(i < 12)
			npc->GetStateMachineManager()->PushStateMachine( *new SM_Runner_3( *npc, i/4, i%4 ), STATE_MACHINE_QUEUE_0, TRUE );
		else
			npc->GetStateMachineManager()->PushStateMachine( *new SM_Announcer( *npc ), STATE_MACHINE_QUEUE_0, TRUE );
	}

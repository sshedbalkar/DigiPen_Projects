...
namespace Wye
{
	...
	const char windowsClassName[] = "WyeEngineWindowClass";
								  ...
								  LPARAM lParam) //The secondary data for the message (if any)
	{
		...
		case WM_CHAR: //A character key was pressed
			{
				//std::cout<<"Dispatching from WM_CHAR: "<<wParam<<"\n";
	...
	bool isUpDown(){ return GetKeyState( VK_UP ) < 0; }
	bool isDownDown(){ return GetKeyState( VK_DOWN ) < 0; }
	bool isLeftDown(){ return GetKeyState( VK_LEFT ) < 0; }
	bool isRightDown(){ return GetKeyState( VK_RIGHT ) < 0; }
}

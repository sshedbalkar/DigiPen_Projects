...
bool BumpMapShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, const D3DXMATRIX& worldMatrix, const D3DXMATRIX& viewMatrix, const
			      D3DXMATRIX& projectionMatrix, ID3D11ShaderResourceView** textureArray, const D3DXVECTOR3& cameraPosition, const std::vector<LightClass>& lights)
{
	bool result;


	// Set the shader parameters that it will use for rendering.
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, textureArray, cameraPosition, lights);
	if(!result)
	{
		return false;
	}

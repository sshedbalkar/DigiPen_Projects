	...
	for(int i=0;  i<count;  i++)  {
		...
		for (int j=0; j<count;  j++) {
...

			{
				...
				float z = cos(phi0);
				scene.SetTexCoord(4.0*theta0/(2.0*PI), 4.0*phi0/PI);
...

			{
				...
				float z = cos(phi0);
				scene.SetTexCoord(4.0*theta1/(2.0*PI), 4.0*phi0/PI);
				scene.SetNormal(Vector3D(x, y, z));
				scene.Vertex(Point3D(r*x, r*y, r*z));
			}

			{
				...
				float z = cos(phi1);
				scene.SetTexCoord(4.0*theta1/(2.0*PI), 4.0*phi1/PI);
				scene.SetNormal(Vector3D(x, y, z));
				scene.Vertex(Point3D(r*x, r*y, r*z));
			}

			{
				...
				float z = cos(phi1);
				scene.SetTexCoord(4.0*theta0/(2.0*PI), 4.0*phi1/PI);
				scene.SetNormal(Vector3D(x, y, z));
				scene.Vertex(Point3D(r*x, r*y, r*z));
			}
		}

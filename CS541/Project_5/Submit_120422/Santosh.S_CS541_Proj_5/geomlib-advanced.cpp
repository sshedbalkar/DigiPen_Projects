			   ...
			   std::pair<Point3D, Point3D> *rpoints)
{
	...
	Vector3D min = box.center - box.extents;
	Vector3D max = box.center + box.extents;
	float t = 0.0f;
	float maxt = -999.0f;
	int maxa = 0;
	...
	for(unsigned a = 0; a < 3; a++)
	{
		times.first[a] = (box.center[a] - ray.origin[a]) / ray.direction[a];
		times.second[a] = (box.extents[a] - ray.origin[a]) / ray.direction[a];
	}

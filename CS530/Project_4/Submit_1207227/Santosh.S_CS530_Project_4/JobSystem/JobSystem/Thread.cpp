...
bool Thread::Initialize(EntryFunction entry, void* instance, cstr threadName)
{
	...
	CheckWin(mHandle != INVALID_HANDLE_VALUE, "Failed to create thread named %s", threadName);
...
{
	if(IsValid())
		VerifyWin(ResumeThread(mHandle), "Failed to resume thread. Thread name: %", mThreadName);
}

void Thread::Suspend()
{
	if(IsValid())
		VerifyWin(SuspendThread(mHandle), "Failed to suspend thread. Thread name: %", mThreadName);
}
...
{
	if(IsValid())
		VerifyWin(CloseHandle(mHandle), "Failed to close thread handle. Thread name: %", mThreadName);
	mHandle = NULL;
}

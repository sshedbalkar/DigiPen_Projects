...
namespace Wye
{
	...
	{
#pragma message("Including Class: WindowsSystem")
	public:
	...
	{
	public:
		MessageCharacterKey(): Message(Msgid::CharacterKey) {};
		int character;
	};
	...
	public:
		...
		MouseButton(MouseButtonIndexId button,bool state, Vec2 position) 
			: Message(Msgid::MouseButton) ,  MouseButtonIndex(button) , ButtonIsPressed(state), MousePosition(position) {};
	...
	{
	public:
		MouseMove(Vec2 position) : Message(Msgid::MouseMove) , MousePosition(position) {};
		Vec2 MousePosition;
	};
	...
	{
	public:
		FileDrop(std::string filename) : Message(Msgid::FileDrop) , FileName(filename) {};
		std::string FileName;
	};
	...
	{
	public:
		MouseWheel(int key, int delta): Message(Msgid::MouseWheel), Key(key), Delta(delta) {};
		int Key;
		int Delta;
	};

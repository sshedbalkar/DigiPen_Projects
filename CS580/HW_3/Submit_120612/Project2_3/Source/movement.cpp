...
bool Movement::ComputePath( int r, int c, bool newRequest )
{
	...
	if( useAStar )
	{
		...
		if( g_astar.calculatePath( curR, curC, r, c, newRequest ))
		{
			const WaypointList* way = g_astar.getPathList();
			m_waypointList.clear();
			//
			for(WaypointList::const_iterator it1 = (*way).begin(), it2 = (*way).end(); it1 != it2; ++it1)
			{
				m_waypointList.push_back( ( *it1 ) );
			}
			return true;
		}
		return false;

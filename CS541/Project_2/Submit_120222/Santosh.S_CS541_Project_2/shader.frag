...
vec4 PhongLightingWithBumpMap()
{
	...
	float d = 1.0f / 256.0f;
	float hv = texture2D(tileHeight, vec2(tVec2.s + d, tVec2.t));
	float hu = texture2D(tileHeight, vec2(tVec2.s, tVec2.t + d));
	//
	// Lookup the height value of the pixel
	float h = texture2D(tileHeight, tVec2);
	//

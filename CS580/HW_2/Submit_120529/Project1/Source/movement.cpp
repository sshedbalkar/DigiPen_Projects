...
void Movement::SetWalkSpeed( float scale )
{
	m_owner->GetBody().SetSpeed( m_speedWalk*scale );
	m_owner->GetTiny().SetMoveKey();
}

void Movement::SetJogSpeed( float scale )
{
	m_owner->GetBody().SetSpeed( m_speedJog*scale );
	m_owner->GetTiny().SetMoveKey();
}

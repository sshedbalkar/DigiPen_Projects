...
bool StaticCircleToStaticRectangle(Vector2D &Center, float Radius, Vector2D &Rect, float Width, float Height)
{
	...
	float fTemp;
	//
	p.x = (Center.x < (fTemp =  Rect.x - (Width / 2))) ? fTemp : Center.x;
	p.x = (Center.x > (fTemp =  Rect.x + (Width / 2))) ? fTemp : p.x;
	p.y = (Center.y < (fTemp =  Rect.y - (Height / 2))) ? fTemp : Center.y;
	p.y = (Center.y > (fTemp =  Rect.y + (Height / 2))) ? fTemp : p.y;
	//
	if(Vector2DSquareDistance(p, Center) < (Radius * Radius))
	{
		return true;
	}
	return false;

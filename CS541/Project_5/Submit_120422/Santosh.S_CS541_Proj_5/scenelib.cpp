...
// $Id$
//
// Operations for building a scenes full of objects, their geometry,
...
void DrawMessage(const char* msg, int line)
{      
	// Get window (i.e., viewport) size.
	int viewport[4];
	...
	glRasterPos2f(4,height-10*line);
...
#ifndef VIEWCELLS // Surrounds from here to end-of-file

void Scene::NewObject(bool closed)
{
	int nObjs = (int)objects.size();
...
void Scene::NewPolygon()
{
	...
	int nPoly = (int)object.polygons.size();
...
void Scene::Hole()
{
	...
	Object& object = objects[nObjs-1];
	int nPoly = (int)object.polygons.size();
	currentStartIndex = (int)object.polygons[nPoly-1].size();
}
...
{
	Vector4D V4 = modeling.Transform(Vector4D(P,1));
	...
	Object& object = objects[nObjs-1];
	int nPoly = (int)object.polygons.size();
	int lPoly = (int)object.polygons[nPoly-1].size();
...
void Texture::set(int i, int j, const Color& c)
{
	assert (channels==3);
...
void Texture::set(int i, int j, const float v)
{
	assert (channels==1);
	float* ptr = texel(i,j,0);
	*ptr = v;
...
ReflTexture::ReflTexture(): Texture(256, 256, 3)
{
	FILE* f;
#ifdef _WIN32
	fopen_s(&f, "sphere.raw", "rb");
#else
	f = fopen("sphere.raw", "rb");
#endif
...
{
	Color ceramic = HSVColor(0.083f, 0.75f, 0.8f);
	Color grout   = HSVColor(0.125f, 0.1f, 0.8f);
	...
	for (int i=0;  i<w;  i++)
		for (int j=0;  j<h;  j++) {
			if (i%n>b && i%n<n-1-b && j%n>b && j%n<n-1-b) {
				...
				float v = float(s(i%n-b)*s(n-1-b-i%n)*s(j%n-b)*s(n-1-b-j%n)/c);
				set(i,j,v); }
			else
				set(i,j,0.0f); }
}
...
{
	Color light = HSVColor(0.125f, 0.7f, 0.8f);
	Color dark  = HSVColor(0.125f, 0.7f, 0.6f);
	Vector3D C(0.0f, 0.0f, 1.0f);
	Vector3D D(0.0f, 1.0f, 0.1f);
	D.normalize();
	
	for (int i=0;  i<w;  i++) {
		float u = (2.0f*i)/w - 1.0f;
		for (int j=0;  j<h;  j++) {
			float v = (2.0f*j)/h - 1.0f;
			Vector3D P = Vector3D(u,v,0.0f)-C;
...
void CreateRect(Scene& scene, float l, float w)
{
	scene.NewObject();
	...
	scene.SetTexCoord(0.0f, 0.0f);
	scene.Vertex(Point3D(-l/2, -w/2,  0.0f));

	scene.SetTexCoord(1.0f, 0.0f);
	scene.Vertex(Point3D( l/2, -w/2,  0.0f));

	scene.SetTexCoord(1.0f, 1.0f);
	scene.Vertex(Point3D( l/2,  w/2,  0.0f));

	scene.SetTexCoord(0.0f, 1.0f);
	scene.Vertex(Point3D(-l/2,  w/2,  0.0f));
...
void CreateCone(Scene& scene, float r, float l, float count)
{
	scene.NewObject();

	for(int i = 0; i < count;  i++)  {
		float theta0 = (i*2.0f*PI)/count;
		float thetah = ((i + 0.5f)*2.0f*PI)/count;
		float theta1 = ((i + 1)*2.0f*PI)/count;
					...
					float r, float l, float count)
{
	...
	for(int i = 0; i < count;  i++)  {
		...
		scene.SetNormal(Vector3D(cos(theta0), sin(theta0), 0.0f));
		scene.Vertex(Point3D(r*cos(theta0), r*sin(theta0), -l/2));
		scene.Vertex(Point3D(r*cos(theta0), r*sin(theta0),  l/2));

		scene.SetNormal(Vector3D(cos(theta1), sin(theta1), 0.0f));
		scene.Vertex(Point3D(r*cos(theta1), r*sin(theta1),  l/2));
		scene.Vertex(Point3D(r*cos(theta1), r*sin(theta1), -l/2)); }

	scene.NewPolygon();
	scene.SetNormal(Vector3D(0.0f, 0.0f, -1.0f));
	for(int i = 0; i < count; i++)  {
		float theta = (i*2.0f*PI)/count;
...
void CreateSphereOctant(Scene& scene, float r, float count)
{
	CreateSphere(scene, r, count, PI/2.0f, PI/2.0f);
}

void CreateSphere(Scene& scene, float r, float count,
				  float theta, float phi)
{
	...
	for(int i=0;  i<count;  i++)  {
		...
		float theta1 = ((i + 1.0f)*theta)/count;

		for (int j=0; j<count;  j++) {
			float phi0 = (j*phi)/count;
			float phi1 = ((j + 1.0f)*phi)/count;
...
void CreateLetterA(Scene& scene)
{
	...
	scene.SetColor(HSVColor(0, 0, 0.5f), HSVColor(0, 0, 0.5f), 0);
	scene.Hole();
	scene.Vertex(top-dy);
	scene.Vertex(rMid-dx+Vector3D(-ax,ay,0));
	scene.Vertex(lMid+dx+Vector3D(ax,ay,0));

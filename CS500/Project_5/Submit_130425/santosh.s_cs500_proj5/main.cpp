...
int main(int argc, char** argv)
{
    ...
    V *image =  new V[scene->width*scene->height];
	V *image2 =  new V[scene->width*scene->height];
	...
	for(int p = 0; p < pass; ++p )
	{
		...
		if( p %5 == 0 )
		{
			...
			{ 
				...
				{
					image2[y*w + x] = image[y*w + x] / (float)p;
				}
			}
			WriteImage(outName, w, h, image2, 1);
		}
	}
	for (unsigned int y = 0; y < h; ++y) 
	{ 
		for (unsigned int x = 0; x < w; ++x) 
		{
			image2[y*w + x] = image[y*w + x] / (float)pass;
		}
	}

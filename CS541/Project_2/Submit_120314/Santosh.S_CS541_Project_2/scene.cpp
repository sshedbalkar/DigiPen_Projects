...
void InitializeScene(Scene &scene)
{
	...
	unsigned char* bytes1 = new unsigned char[256*256*3];
	unsigned char* bytes2 = new unsigned char[256*256*1];
	FILE* f = fopen("tile_color.raw", "rb");
	if(f)
	{
		fread(bytes1, 256*256, 3, f);
		fclose(f);
	}
	f = fopen("tile_height.raw", "rb");
	if(f)
	{
		fread(bytes2, 256*256, 1, f);
		fclose(f);
	}
	...
	gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGB, 256, 256, GL_RGB, GL_BYTE, bytes1);
	//
	texId2 = glGenLists(1);
    glBindTexture(GL_TEXTURE_2D, texId2);
    glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	gluBuild2DMipmaps(GL_TEXTURE_2D, GL_LUMINANCE, 256, 256, GL_LUMINANCE, GL_BYTE, bytes2);
	//
	delete[] bytes1;
	delete[] bytes2;
}
...
void DrawScene(Scene &scene)
{
    ...
    loc = glGetUniformLocation(shader.program, "tileColor");
    glUniform1i(loc, 0);
	loc = glGetUniformLocation(shader.program, "tileHeight");
    glUniform1i(loc, 1);
